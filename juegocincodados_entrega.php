...
function calcularPuntos( array $tdados): int
{
    ...
    foreach ($tdados as $td) {
        ...
        $puntos -= ($valormin + $valormax);
    
   return $puntos;

   /** 
    * También se puede
    * $puntos = array_sum($tdados);
    * $valormin = min($tdados);
    * $valormax = max($tdados);
    * return $puntos - ($valormin + $valormax);
    */
}
